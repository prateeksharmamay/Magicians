...
public class BackCurtain extends Actor
{
    ...
    {       
        ...
        NextButton nextButton = playWorld.getNextButton();
        if (playWorld.getGrid().isExpanded() && nextButton.isPressed())
        {
            if (direction)
            {
                animationDown();
    ...
    {
        PlayWorld theCurtainScenario = (PlayWorld) getWorld();
        NextButton nextButton = theCurtainScenario.getNextButton();
        if (counter % 20 == 0)
        {
            imageNumber = (imageNumber + 1) % images.length;
            ...
            if (imageNumber == 6)
            {
                ...
                getWorld().setPaintOrder(Card.class,NextButton.class, FrontCurtain.class, BackCurtain.class);
                startTimerForGame();
